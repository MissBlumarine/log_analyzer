...
File = collections.namedtuple('File', 'path')


def get_logfile(config):
    path_log = config.get("LOG_DIR")
    ...
    # pattern = f"nginx-access-ui.log-{date_today}.gz"
    pattern = f"nginx-access-ui.log-*.gz"
    list_of_files = []
    for file in os.listdir(path_log):
        if fnmatch.fnmatch(file, pattern):
            list_of_files.append(file)
        else:
            logging.info("No files to analyze was found")
    logfile = max(list_of_files)
    path = os.path.join(os.path.abspath(path_log), logfile)
    logging.info(f"Choised log filename is {logfile}")
    logfile = File(path)
    print(logfile)
    return logfile


def get_file_fill(file):
    fill = gzip.open(file.path, 'rt', encoding='utf-8')
    # list=[]
    with fill:
        for line in fill:
            # list.append(line)
...
def tre(config):
    ...
    lines = get_file_fill(logfile)

tre(config)


# def parse(file_strings):
...
#
# def main():
#     file = get_logfile(config)
#     line = get_file_fill(file)
#     print(line)
#
#
# if __name__ == 'main':
